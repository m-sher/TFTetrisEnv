...
class TFScorer:
    def __init__(self):
        self._corner_cells = tf.constant(
            [[0, 0], [0, 2], [2, 2], [2, 0]], dtype=tf.int64
        )
        ...
        self._tspin_mini_attack = tf.constant(
            [0, 0, 1, 0, 0], tf.float32
        )  # no tspin mini triple
        ...
        with tf.name_scope("Judge"):
            ...
            def process_t_piece():
                def check_t_spin():
                    ...
                    corners = tf.reduce_any(
                        [
                            tf.reduce_any(
                                corner_coords >= tf.shape(board, out_type=tf.int64),
                                axis=-1,
                            ),
                            tf.reduce_any(corner_coords < 0, axis=-1),
                        ],
                        axis=0,
                    )

                    # Check in bounds corners
                    corner_coords = tf.maximum(
                        tf.cast(0, tf.int64),
                        tf.minimum(
                            corner_coords,
                            [
                                tf.shape(board, out_type=tf.int64)[0] - 1,
                    ...
                    corners = tf.cast(
                        tf.reduce_any(
                            [corners, tf.gather_nd(board, corner_coords) != 0], axis=0
                        ),
                        tf.float32,
                    )
                    ...
                    filled_fronts = tf.reduce_any(
                        tf.reduce_all(
                            self._t_fronts[:, None, :] == piece_cells[None, :, :],
                            axis=-1,
                        ),
                        axis=-1,
                    )
                    ...
                    return tf.cond(
                        tf.logical_and(front_corners == 2, back_corners >= 1),
                        lambda: TFSpins.T_SPIN,
                        lambda: tf.cond(
                            tf.logical_and(front_corners == 1, back_corners == 2),
                            lambda: tf.cond(
                                tf.reduce_sum(tf.abs(piece_delta_loc)) > 2,
                                lambda: TFSpins.T_SPIN,
                ...
                return tf.cond(
                    piece_delta_r != 0, check_t_spin, lambda: TFSpins.NO_SPIN
                )

            spin = tf.cond(
                piece_type == TFPieceType.T, process_t_piece, lambda: TFSpins.NO_SPIN
            )
            ...
            b2b, combo = tf.cond(
                ...
                lambda: tf.cond(
                    (
                        tf.logical_or(
                            tf.logical_or(spin != TFSpins.NO_SPIN, clears == 4),
                            perfect_clear,
                        )
                    ),

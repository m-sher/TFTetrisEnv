...
class TFRotationSystem:
    ...
    def overlaps(self, cells, loc, board) -> bool:
        with tf.name_scope("Overlaps"):
            ...
            outside_v = tf.logical_or(
                tf.reduce_any(cell_coords[:, 0] < 0),
                tf.reduce_any(cell_coords[:, 0] > board.shape[0] - 1),
            )

            # Outside board horizontally
            outside_h = tf.logical_or(
                tf.reduce_any(cell_coords[:, 1] < 0),
                tf.reduce_any(cell_coords[:, 1] > board.shape[1] - 1),
            )
    ...
    def kick_piece(
        ...
        with tf.name_scope("Kick"):
            kick_table = tf.cond(
                piece_type == TFPieceType.I, lambda: self.i_kicks, lambda: self.kicks
            )
            ...
            def check_kick_body(kicked, delta_loc, i):
                kicked, delta_loc, i = tf.cond(
                    tf.reduce_all(
                        delta_loc_candidates[i] == tf.zeros((2,), dtype=tf.int64)
                    ),
                    lambda: (
                        tf.constant(False),
                        delta_loc,
                        tf.shape(delta_loc_candidates, out_type=tf.int64)[0],
                    ),
                    lambda: tf.cond(
                        self.overlaps(
                            cells=new_cells,
                            loc=piece_loc + delta_loc_candidates[i],
                            board=board,
                        ),
                        lambda: (tf.constant(False), delta_loc, i + 1),
                        lambda: (
                            tf.constant(True),
                            delta_loc_candidates[i],
                            tf.shape(delta_loc_candidates, out_type=tf.int64)[0],
                        ),
                    ),
                )
            ...
            kicked, delta_loc, delta_loc_ind = tf.while_loop(
                lambda k, d, i: tf.logical_and(
                    tf.logical_not(k),
                    i < tf.shape(delta_loc_candidates, out_type=tf.int64)[0],
                ),
                check_kick_body,
            ...
            return tf.cond(
                kicked,
                lambda: (
                    tf.identity(piece_loc + delta_loc),

...
class TFTetrisEnv(py_environment.PyEnvironment):
    ...
    def __init__(self, queue_size, max_holes, max_len=9, seed=[0, 0]):
        ...
        self._board = tf.Variable(name='board', initial_value=tf.zeros((24, 10), dtype=tf.float32), dtype=tf.float32)

        self._last_heights = tf.Variable(name='last_heights', initial_value=tf.zeros((), dtype=tf.float32), dtype=tf.float32)
        self._last_holes = tf.Variable(name='last_holes', initial_value=tf.zeros((), dtype=tf.float32), dtype=tf.float32)
        self._last_skyline = tf.Variable(name='last_skyline', initial_value=tf.zeros((), dtype=tf.float32), dtype=tf.float32)
        self._last_bumpy = tf.Variable(name='last_bumpy', initial_value=tf.zeros((), dtype=tf.float32), dtype=tf.float32)

        self._hold_piece = tf.Variable(name='hold_piece', initial_value=tf.fill(dims=(), value=TFPieceType.N), dtype=tf.int64)

        all_pieces = tf.range(1, 8, dtype=tf.int64)
        self._next_bag = tf.Variable(name='next_bag',
                                     initial_value=self._stateless_shuffle(all_pieces),
                                     dtype=tf.int64)
        self._bag_ind = tf.Variable(name='bag_ind', initial_value=tf.zeros((), dtype=tf.int64), dtype=tf.int64)
        
        self._active_piece_type = tf.Variable(name='active_piece_type', initial_value=tf.fill(dims=(), value=TFPieceType.N), dtype=tf.int64)
        self._active_loc = tf.Variable(name='active', initial_value=tf.zeros((2,), dtype=tf.int64), dtype=tf.int64)
        self._active_delta_loc = tf.Variable(name='active', initial_value=tf.zeros((2,), dtype=tf.int64), dtype=tf.int64)
        self._active_r = tf.Variable(name='active', initial_value=tf.zeros((), dtype=tf.int64), dtype=tf.int64)
        self._active_delta_r = tf.Variable(name='active', initial_value=tf.zeros((), dtype=tf.int64), dtype=tf.int64)
        self._active_cells = tf.Variable(name='active', initial_value=tf.zeros((4, 2), dtype=tf.int64), dtype=tf.int64)
        
        to_spawn = tf.gather(self._next_bag,
                             self._bag_ind)
        self._spawn_piece(to_spawn)
        self._bag_ind.assign_add(tf.ones_like(self._bag_ind))
        
        self._queue = tf.Variable(name='queue', initial_value=tf.fill(dims=(self._queue_size,), value=TFPieceType.N), dtype=tf.int64)
        self._fill_queue()

        self._episode_ended = tf.Variable(name='episode_ended', initial_value=False, dtype=tf.bool)

        self._observation_spec = {
            'board': tensor_spec.BoundedTensorSpec(
                shape=(24, 10, 1), dtype=tf.float32, minimum=0.0, maximum=1.0, name='board'),
            'pieces': tensor_spec.BoundedTensorSpec(
                shape=(2 + self._queue_size,), dtype=tf.int64, minimum=0, maximum=7, name='pieces')
        }
        ...
        self._reward_spec = {
            'attack': tensor_spec.TensorSpec(
                shape=(), dtype=tf.float32, name='attack'),
    ...
    def _reset(self):
        with tf.name_scope('Reset'):
            ...
            self._next_bag.assign(self._stateless_shuffle(tf.range(1, 8, dtype=tf.int64)))
            ...
            self._recent_time_step = ts.TimeStep(step_type=ts.StepType.FIRST,
                                                 reward=reward,
                                                 discount=1.0,
    ...
    def _step(self, key_sequence):
        ...
        with tf.name_scope('Step'):
            ...
            def do_step():
                ...
                death_penalty = tf.cond(self._episode_ended,
                ...
                time_step = tf.cond(self._episode_ended,
                                    lambda: ts.termination(observation=observation,
                                                           reward=reward),
                                    lambda: ts.transition(observation=observation,
                                                          reward=reward))
                return time_step
            
            self._recent_time_step = tf.cond(self._episode_ended,
                                             self.reset,
                                             do_step)
    ...
    def _press_key(self, key, top_out, attack):
        with tf.name_scope('PressKey'):
            ...
            tf.cond(key == TFKeys.HOLD,
                    self._try_hold,
                    lambda: self._try_key_vector(key_vector))
            
            top_out, attack = tf.cond(key == TFKeys.HARD_DROP,
                                      self._lock_and_judge,
                                      lambda: (top_out, attack))
    ...
    def _lock_and_judge(self):
        ...
        attack = self._scorer.judge(piece_type=self._active_piece_type, piece_loc=self._active_loc,
                                    piece_delta_loc=self._active_delta_loc, piece_delta_r=self._active_delta_r,
                                    piece_cells=self._active_cells, board=self._board, clears=clears)
        return top_out, attack

    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(), dtype=tf.int64, name='piece_type')
    ])
    def _spawn_piece(self, piece_type):
        ...
        with tf.name_scope('SpawnPiece'):
            ...
            self._active_cells.assign(tf.gather_nd(self._rotation_system.orientations,
    ...
    def _create_observation(self) -> dict[str, tf.Tensor]:
        with tf.name_scope('CreateObservation'):
            ...
            pieces = tf.cast(tf.concat([active_piece_type,
                                        hold_piece,
                                        queue], axis=0), tf.int64)
    ...
    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(3,), dtype=tf.int64, name='key_vector')
    ])
    def _try_key_vector(self, key_vector):
        ...
        with tf.name_scope('TryKeyVector'):
            ...
            tf.cond(try_delta_r != 0,
                    lambda: self._rotate(try_delta_r),
                    lambda: self._move(try_delta_loc))

    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(), dtype=tf.int64, name='try_delta_r')
    ])
    def _rotate(self, try_delta_r):
        ...
        with tf.name_scope('Rotate'):
            ...
            new_cells = self._rotation_system.orientations[self._active_piece_type][new_r]

            new_loc, new_delta_loc, new_r, new_delta_r, new_cells = tf.cond(
                self._rotation_system.overlaps(cells=new_cells, loc=self._active_loc, board=self._board),
                lambda: self._rotation_system.kick_piece(piece_type=self._active_piece_type,
                                                         piece_loc=loc,
                                                         ...
                                                         new_cells=new_cells,
                                                         board=self._board),
                lambda: (loc, tf.zeros((2,), dtype=tf.int64), new_r, try_delta_r, new_cells)
            )
    ...
    def _move(self, try_delta_loc):
        ...
        with tf.name_scope('Move'):
            ...
            def can_move(delta_loc):
                should_try = tf.logical_or(tf.abs(delta_loc[0]) < tf.abs(try_delta_loc[0]),
                                           tf.abs(delta_loc[1]) < tf.abs(try_delta_loc[1]))
                new_loc = self._active_loc + delta_loc + direction
                would_overlap = self._rotation_system.overlaps(cells=self._active_cells, loc=new_loc, board=self._board)

                return tf.logical_and(should_try, tf.logical_not(would_overlap))

            delta_loc = tf.while_loop(can_move,
            ...
            new_delta_r = tf.cond(tf.reduce_any(delta_loc != 0),
    ...
    def _try_hold(self):
        with tf.name_scope('TryHold'):
            ...
            to_spawn, new_queue, bag_ind, next_bag = tf.cond(
                ...
                lambda: (tf.identity(self._hold_piece),
    ...
    def _lock_piece(self) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        with tf.name_scope('LockPiece'):
            ...
            # indices -> (4, 2)
            # updates -> (4,)   
            new_board = tf.tensor_scatter_nd_update(self._board,
                                                    cell_coords,
    ...
    def _fill_queue(self):
        ...
        with tf.name_scope('FillQueue'):
            ...
            queue = tf.identity(self._queue)
            new_queue = tf.while_loop(lambda queue: tf.reduce_sum(tf.cast(queue == TFPieceType.N, tf.float32)) > 0,
                                      hard_pop,
                                      [queue],
                                      parallel_iterations=1)[0]
            self._queue.assign(new_queue)
    ...
    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(5,), dtype=tf.int64, name='queue')
    ])
    ...
    def _get_from_bag(self):
        ...
        with tf.name_scope('GetFromBag'):
            bag_ind, next_bag = tf.cond(self._bag_ind >= tf.shape(self._next_bag, out_type=tf.int64)[0],
                                        lambda: (tf.zeros((), dtype=tf.int64), self._stateless_shuffle(self._next_bag)),
    ...
    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(7,), dtype=tf.int64, name='tensor')
    ])
    def _stateless_shuffle(self, tensor):
        ...
        with tf.name_scope('StatelessShuffle'):
            ...
            logits = tf.random.stateless_uniform(shape=(size,),
    ...
    def _clear_lines(self) -> tf.Tensor:
        ...
        with tf.name_scope('ClearLines'):
            ...
            # Clear filled rows
            rows_cleared_board = tf.where(rows_to_clear[:, None],
                                          empty_board,
                                          board)
            # Drop down the rows above using stable argsort
            row_inds = tf.argsort(tf.cast(rows_to_keep, tf.int64),
                                  axis=0,
                                  direction='ASCENDING',
                                  stable=True)
    ...
    def _get_heights(self) -> tf.Tensor:
        ...
        with tf.name_scope('GetHeights'):
            height_matrix = tf.range(self._board.shape[0], 0, -1, dtype=tf.float32)[..., None]
            heights = tf.reduce_max(self._board * height_matrix, axis=0)

            return heights

    @tf.function(jit_compile=True, input_signature=[
        tf.TensorSpec(shape=(10,), dtype=tf.float32, name='heights')
    ])
    ...
    def _get_skyline(self, heights) -> tf.Tensor:
        ...
        with tf.name_scope('GetSkyline'):
            ...
            skyline = tf.reduce_sum(sorted_heights[-num_cols//2:]) - tf.reduce_sum(sorted_heights[:num_cols//2])
            return skyline
